...
(function () {
  ...
  function runTo(target) {
    ...
    if (normalized === 'deid') {
      const mini = byId('deid-mini-report');
      if (mini && !mini.open) mini.open = true;
    }
    collapseAll();
    expand(normalized, true);
  }
  ...
  function onDeidResponse(evt) {
    if (!evt || !evt.detail || !evt.detail.xhr) return;
    try {
      const data = JSON.parse(evt.detail.xhr.responseText || '{}');
      if (typeof data.text === 'string') {
        setDeidText(data.text);
      }
      const host = byId('deid-report');
      if (host) {
        const changes = Array.isArray(data.changes) ? data.changes : [];
        if (changes.length) {
          const items = changes
            .slice(0, 200)
            .map((c) => `<li><code>${escapeHtml(c.field || '')}</code></li>`)
            .join('');
          host.innerHTML = `<ul>${items}</ul>`;
        } else {
          host.innerHTML = '<span class="muted">No changes detected.</span>';
        }
      }
    } catch (err) {
      console.warn('Failed to process de-identify response', err);
    }
  }
