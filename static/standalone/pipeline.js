(function () {
  ...
  async function fillTriggers() {
    ...
    const version = select.value || '2.5';
    try {
  ...
  function parseCount() {
    const raw = $1(document, `#${COUNT_INPUT_ID}`)?.value || '1';
  ...
  async function loadSample() {
    ...
    const version = versionSelect?.value || '2.5';
  ...
  function onReady() {
    ...
    const countInput = $1(document, `#${COUNT_INPUT_ID}`);
    if (countInput) {
      const syncCount = () => {
        const normalized = parseCount();
        countInput.value = String(normalized);
        toggleAllActionTrays();
      };
      countInput.addEventListener('change', syncCount);
      countInput.addEventListener('input', () => {
        // allow typing freely but keep tray state responsive
        toggleAllActionTrays();
      });
      syncCount();
    }
